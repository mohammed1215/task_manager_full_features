...
import { ActivityTypes, WorkspaceMemberRoles } from '../enum/enum';
...
export class CommentService {
    constructor(
        ...
        private readonly boardRepo: Repository<BoardMember>,
        private readonly boardService: BoardService,
        @InjectRepository(WorkspaceMember)
        private readonly workMemberRepo: Repository<WorkspaceMember>,
    ...
    async create(
        ...
        const task = await this.taskRepo.findOne({
            where: { id: taskId },
            relations: ['board', 'board.workspace', 'column', 'assignedTasks'],
        });
        if (!task) throw new NotFoundException('task not found');

        // check if user is member+ in the board or admin or owner of the workspace
        const isBoardMember = await this.boardRepo.findOne({
            where: {
        ...
        const isWorkspaceMemberAdminOrOwner = await this.workMemberRepo.findOne(
            {
                where: [
                    {
                        user: { id: userId },
        ...
        await this.activityService.create({
            activityType: ActivityTypes.commented,
            fieldName: 'comment',
            taskId,
            actorId: userId,
            oldValue: null,
            newValue: { comment: { content: comment.content } },
        });
        return savedComment;
    }
    ...
    async update(
        ...
        if (
            ...
            throw new BadRequestException(
                'can not update comment after 15m of creation',
            );
        }
        ...
        await this.commentRepo.update(
            { id: commentId },
            { ...updateCommentData, isEdited: true },
        );
    ...
    async remove(userId: string, commentId: string) {
        ...
        //get user
        const workspaceMember = await this.workMemberRepo.findOne({
            where: {
                user: { id: userId },
                workspace: { id: comment.task.board.workspace.id },
            },
        });
        if (!workspaceMember)
        ...
        if (
            ...
            throw new ForbiddenException(
                'not allowed privilages: only author of the comment or owner or admin of the workspace can delete the comment',
            );
        }
