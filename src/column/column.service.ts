...
export class ColumnService {
    ...
    async create(
        ...
        if (boardMember.role !== BoardRoles.ADMIN) {
            throw new ForbiddenException(
                'Not Allowed Privilages: Only Admin Are allowed',
            );
        }
        ...
        if (columnCount >= 10) {
            throw new BadRequestException(
                `number of columns are max (${columnCount}/10)`,
            );
        }
    ...
    async update(
        ...
        const result = await this.columnRepo.update(
            { board: { id: boardId }, id: columnId },
            { ...updateColumnDto },
        );
    ...
    async reorder(
        ...
        const member = await this.boardService.findOneBoardMember(
            userId,
            boardId,
        );
        ...
        const column = await this.columnRepo.findOne({
            where: { id: reOrderColumnDto.columnId },
        });
        if (!column) throw new NotFoundException('column does not exist');

        const from =
            reOrderColumnDto.newPosition >= column.position
                ? column.position + 1
                : reOrderColumnDto.newPosition;
        const to =
            reOrderColumnDto.newPosition >= column.position
                ? reOrderColumnDto.newPosition
                : column.position - 1;
        ...
        const columns = await this.columnRepo.find({
            where: { position: Between(from, to), board: { id: boardId } },
        });
    ...
    async remove(userId: string, boardId: string, columnId: string) {
        ...
        const taskCount =
            await this.taskService.getCountOfTasksForSpecificColumn(
                boardId,
                columnId,
            );
        if (taskCount)
            throw new BadRequestException(
                'Can not delete column with existing tasks',
            );

        // delete column
        const deletedResult = await this.columnRepo.delete({
            id: columnId,
            board: { id: boardId },
        });
