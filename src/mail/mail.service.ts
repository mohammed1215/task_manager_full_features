    ...
    return this.mailService
      .sendMail({
        ...
        html: `<div>Reset Password
             <a href="${this.config.get('FRONTEND_URL')}/reset-password?token=${resetToken}&id=${userId}">click here</a>
             if button didn't work
             <a href="${this.config.get('FRONTEND_URL')}/reset-password?token=${resetToken}&id=${userId}">
             ${this.config.get('FRONTEND_URL')}/reset-password?token=${resetToken}&id=${userId}
             </a>
            </div>`,
      })
      .then((success) => {
  ...
  async sendInvitationEmail(
    ...
    const result = await this.mailService.sendMail({
      ...
      html: `
            ...
            <a href="${this.config.get('FRONTEND_URL')}/accept-invite?invitationId=${invitationId}&senderId=${senderId}" style="background-color: green;">Accept</a>
            <a href="${this.config.get('FRONTEND_URL')}/decline-invite?invitationId="${invitationId}&senderId=${senderId}" style="background-color: red;">Decline</a>
            </div>
            `,
    });
    if (result.rejected.length > 0)
