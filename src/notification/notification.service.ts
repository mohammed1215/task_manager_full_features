...
export class NotificationService {
    ...
    async create(userId: string, createNotificationDto: CreateNotificationDto) {
        ...
        const notificationCount2 = await this.notificationRepo.count({
    ...
    async markAsRead(userId: string, notificationId: string) {
        this.notificationRepo.update(
            { user: { id: userId }, id: notificationId, isRead: false },
            { isRead: true },
        );
    ...
    async autoMarkOldNotificationsAsRead() {
        ...
        const result = await this.notificationRepo.update(
            { isRead: false, createdAt: LessThan(sevenDaysAgo) },
            { isRead: true },
        );
        console.log(
            'Automatically marked 7-day old notifications as read. Number of affected rows ' +
                result.affected,
        );
    }
