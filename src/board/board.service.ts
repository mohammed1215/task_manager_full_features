...
export class BoardService {
    ...
    try {
      ...
      if (createBoardDto.visibility === Visibility.PUBLIC) {
        ...
        if (workspaceMembers.length > 0) {
          const createdBoardMembers = workspaceMembers.map((member) => ({
          ...
          await queryRunner.manager.save(BoardMember, boardMembersEntities);
        }
      }
  ...
  findAll(
    ...
    const archivedFilter =
      filter === 'active' ? false : filter === 'archived' ? true : undefined;
