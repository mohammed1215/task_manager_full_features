...
export class BoardService {
  ...
  async create(userId:string,workspaceId:string,createBoardDto: CreateBoardDto) {
    //check if user in workspace
    const member = await this.workspaceMemberService.findOne(workspaceId,userId)
    ...
    try {
      //create board 
      const board =  queryRunner.manager.create(Board,{...createBoardDto,createdBy:{id:userId},workspace:{id:workspaceId}})
      ...
      const boardMember = queryRunner.manager.create(BoardMember,{board: {id: board.id},role:BoardRoles.ADMIN,invitedBy:{id:userId},user:{id:userId}})
  ...
  async archiveBoard(userId:string,boardId:string){
    const user = await this.checkAdminRoleOfBoardMemberUser(userId,boardId,'Not Allowed Privilages: Admins are the only ones that can archive the board')
    ...
    try {
      ...
      const updateResult = await queryRunner.manager.update(Board,{id:boardId},{isArchived:true,archivedAt:new Date()})
      const deletedResult = await queryRunner.manager.softDelete(Board,{id:boardId})
      
      if(!updateResult.affected) throw new NotFoundException('board not found')
      if(!deletedResult.affected) throw new NotFoundException('board not found')
  ...
  async restoreBoard(userId:string,boardId:string){
    ...
    const exceededDate =  new Date(board.archivedAt).getTime()+30*24*60*60*1000 
    if(exceededDate && new Date().getTime() > exceededDate){
      throw new BadRequestException('30 days has been exceeded')
    }
    try {

      const restoreResult = await queryRunner.manager.restore(Board,{id:boardId})
      const updateResult = await queryRunner.manager.update(Board,{id:boardId},{isArchived:false,archivedAt:null})
      
      if(!updateResult.affected) throw new NotFoundException('board not found')
      if(!restoreResult.affected) throw new NotFoundException('board not found')
  ...
  findAll(userId:string,workspaceId:string,page:number,limit:number,filter:'archived'|'active'|'all') {
    limit = Math.max(1,limit)
    page = Math.max(1,page)
    const archivedFilter = filter === 'active' ? false : filter === 'archived' ? true : undefined;
    ...
    const baseCondition = {
     ...
     return this.boardRepo.find({where: [
      {
  ...
  async findOneBoard(boardId: string) {
    const board = await this.boardRepo.findOne({where:{id:boardId},withDeleted:true,relations:['createdBy','workspace']});
    if(!board) throw new NotFoundException('board not found')
    return board
  }

  async update(userId: string,boardId:string, updateBoardDto: UpdateBoardDto) {
    const user = await this.checkAdminRoleOfBoardMemberUser(userId,boardId,'Not Allowed Privilages: Admins are the only ones that can update the board')

    const result = await this.boardRepo.update({id:boardId},{...updateBoardDto})
  ...
  async remove(userId:string,boardId: string,confirmation:string) {
    ...
    if(board.createdBy.id !== userId && board.archivedAt){
      throw new ForbiddenException('owners of the board are the ones that can delete archived boards')
    } 
    
    const member = await this.findOneBoardMember(userId,boardId)
    if(member.role !== BoardRoles.ADMIN){
      throw new ForbiddenException('Not Allowed Privilages: Admins are the only ones that can delete the board')
    }
  ...
  async findOneBoardMember(userId:string,boardId:string){
    const member = await this.boardMemberRepo.findOne({where:{board:{id:boardId},user:{id:userId}},withDeleted:true})
    if(!member) throw new NotFoundException('member does not exist on board')
    return member  
  }

  async checkAdminRoleOfBoardMemberUser(userId:string,boardId:string,message:string){
