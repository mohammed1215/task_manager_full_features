...
export class UserService {
    ...
    async findOne(userId: string) {
        const user = await this.userRepo.findOne({
            where: { id: userId },
            relations: ['sentInvitations', 'receivedInvitation', 'memberships'],
        });
        if (!user) {
    ...
    async updateNotificationPreferences(
        ...
        const result = await this.userRepo.update(
            ...
            {
                emailPreference:
                    updateNotificationPreferenceDto.emailPreference,
            },
        );
        ...
        return { message: 'email preference updated successfully' };
    }
