...
export class AuthService {
    ...
    async register(createUserDto:CreateUserDto){
        ...
        const verifyToken = await this.jwtProvider.generateVerificationToken({userId:user.id,email:user.email})
        await this.mailService.sendVerificationMail(user.email,verifyToken,user.id)
        return user
    }
    ...
    async login(loginDto:LoginDto){
        ...
        const refreshToken = await this.jwtService.signAsync({userId:user.id,email:user.email},{
            expiresIn: '30d'
        })
    ...
    async verifyEmail(verifyToken:string,userId:string){
        ...
        const payload = await this.jwtService.verifyAsync(verifyToken,{secret: this.config.get<string>('VERIFICATION_SECRET_KEY')})

        // check if the userId === token.userId
        if(userId !== payload.userId) throw new BadRequestException('not allowed user')
    ...
    async forgotPassword(email:string){
        ...
        const result = await this.mailService.sendForgotPasswordMail(email,user.id)
        if(result.rejected.length > 0 ) throw new BadRequestException(`couldn't send the email try again`)
        return {
    ...
    async resetPassword(resetToken:string,userId:string,newPassword:string){
        //validate token
        const payload = await this.jwtService.verifyAsync(resetToken,{secret: this.config.get<string>('RESET_SECRET_PASS')})
