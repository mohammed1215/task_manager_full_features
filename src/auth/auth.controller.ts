...
export class AuthController {
    ...
    @Post('register')
    @ApiOperation({ summary: 'Register a new user', description: 'Create a new user account and send verification email' })
   ...
   async register(@Body() createUserDto:CreateUserDto){
    ...
    @Post('login')
    @ApiOperation({ summary: 'User login', description: 'Login with email and password to receive JWT token' })
    ...
    @ApiOperation({ summary: 'Request password reset', description: 'Send password reset link to user email' })
    ...
    @ApiResponse({ status: 400, description: 'Invalid token or password not provided' })
    @ApiQuery({ name: 'token', type: 'string', description: 'Reset token from email' })
    @ApiQuery({ name: 'id', type: 'string', description: 'User ID' })
    @ApiBody({ schema: { properties: { password: { type: 'string', example: 'newPassword123' } } } })
    ...
    @Post('verify-email')
    @ApiOperation({ summary: 'Verify email', description: 'Verify user email using token sent to email' })
    @ApiResponse({ status: 200, description: 'Email verified successfully' })
    @ApiResponse({ status: 400, description: 'Invalid verification token' })
    @ApiQuery({ name: 'token', type: 'string', description: 'Verification token from email' })
    @ApiQuery({ name: 'id', type: 'string', description: 'User ID' })
