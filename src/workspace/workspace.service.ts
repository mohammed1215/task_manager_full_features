...
export class WorkspaceService {
  ...
  async remove(workspaceId: string, userId: string, currentUserId: string) {
    ...
    const owner = await this.workspaceMemberRepo.findOne({
      where: { workspace: { id: workspaceId }, user: { id: currentUserId } },
    });
    if (!owner) throw new NotFoundException('owner not found');
  ...
  async inviteMember(
    ...
    if (existedUser)
      throw new BadRequestException('user already exists in the workspace');
  ...
  async acceptInvitation(
    ...
    try {
      ...
      let boardMembers = publicBoards.map((board) => ({
        ...
        invitedBy: invitation.sender?.id ? { id: invitation.sender?.id } : null,
      }));

      if (boardMembers.length) {
        boardMembers = queryRunner.manager.create(BoardMember, boardMembers);
